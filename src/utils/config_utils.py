...
class BaseConfig:
    ...
    force_fact_extraction_from_scratch: bool = field(
        default=False,
        metadata={"help": "If set to True, will ignore all existing fact result files and rebuild them from scratch."}
    )
    ...
    save_fact_results: bool = field(
        default=True,
        metadata={"help": "If set to True, will save extracted fact and event results to disk."}
    )
    ...
    fact_sim_threshold: float = field(
        default=0.80,
        metadata={"help": "Similarity threshold for considering two facts as related."}
    )
    related_fact_top_k: int = field(
        default=5, 
        metadata={"help": "Number of top related facts to consider for each fact."}
    )
