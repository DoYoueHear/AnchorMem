...
class TransformersEmbeddingModel(BaseEmbeddingModel):
    """
    To select this implementation you can initialise AnchorMem with:
    ...
    def encode(self, texts: List[str], instruction=None) -> None:
        try:
            # if "all-MiniLM-L6" in self.model_id:
            response = self.model.encode(texts, batch_size=self.batch_size)
            # else:
                # response = self.model.encode(texts, batch_size=self.batch_size, prompt=instruction)
    ...
    def batch_encode(self, texts: List[str], **kwargs) -> None:
        if len(texts) < self.batch_size:
            return self.encode(texts,kwargs.get('instruction', None))
