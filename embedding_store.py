...
from src.utils.misc_utils import compute_mdhash_id
...
class EmbeddingStore:
    ...
    def insert_strings(self, texts: List[str]):
        ...
        for text in texts:
            nodes_dict[compute_mdhash_id(text, prefix=self.namespace + "-")] = {'content': text}
            # print(f"processing {self.namespace} text: {text}")
        ...
        # texts_to_encode = [nodes_dict[hash_id]["content"] for hash_id in missing_ids]
        # if self.namespace in ["fact", "thing"]:
        #     list_of_facts = [list(ast.literal_eval(s)) for s in texts_to_encode]
        #     new_list_of_facts = [fact[0:3] if len(fact) >= 3  else fact for fact in list_of_facts]
           
        #     texts_to_encode_for_embedding = [" ".join(fact) for fact in new_list_of_facts]
        #     missing_embeddings = self.embedding_model.batch_encode(texts_to_encode_for_embedding)
        # else:
